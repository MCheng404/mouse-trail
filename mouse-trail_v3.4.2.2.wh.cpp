...
// 提亮：等价于朝白色插值（复用 LerpColor，避免重复的插值表达式）
// Lighten: equivalent to lerping toward white (reuse LerpColor to avoid duplicated lerp expressions)
static D2D1_COLOR_F LighterColor(D2D1_COLOR_F c, float amount = 0.55f) {
    return LerpColor(c, kColorWhite, amount);
}
...
static void UpdateConstantBuffer(int width, int height, const GradData* cols = nullptr, bool force = false) {
    ...
    float* data = buf;             // 先写入栈上缓冲，再与缓存比较 / write to stack buffer first, then compare with cache
        ...
        // 文字描边采样步长（图集UV空间）与颜色/透明度/开关 / Text stroke sample step (atlas UV) and color/alpha/toggle
        {
            ...
            // P2-6：noiseSeed（第 94 个 float）不在块内直接取值 —— 它需要先把「除自己以外的 448 字节」与上次上传内容比对后
            // 才能决定是推进还是复用，解析见下方 / resolved after comparing the rest of the buffer with the cache, see below
            data[95] = 0.0f;                                                 // _pad3
        }
    ...
    // 布局不变：仍然写第 94 个 float（HLSL cb0[23].z），cbuffer ByteWidth 依旧 448 字节。
    // Layout unchanged: still float #94 (HLSL cb0[23].z), ByteWidth stays 448.
    {
        const int kSeedIdx = 94;                                        // float 下标 94 → cb0[23].z（4*23=92 → z=94）
        const size_t headBytes = (size_t)kSeedIdx * sizeof(float);      // 第 0~93 项
        const size_t tailBytes = sizeof(buf) - headBytes - sizeof(float); // 第 95~111 项
        // 只有 g_cbCacheValid 且「除种子自身外」的所有项都一致才算内容未变，所以拆成两段 memcmp（跳过 data[94]）
        // Content is "unchanged" only if every field except the seed matches → two-segment memcmp skipping float #94.
        bool contentUnchanged = g_cbCacheValid &&
            memcmp(buf, g_cbCache, headBytes) == 0 &&
            memcmp(buf + kSeedIdx + 1, g_cbCache + kSeedIdx + 1, tailBytes) == 0;
        data[kSeedIdx] = ResolveNoiseSeed(contentUnchanged, g_cbCache[kSeedIdx]);
    }
